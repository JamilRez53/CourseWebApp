...
//     await fetch("http://localhost:5000/result/getResult",{
//             method:"GET",
//          }).then((res) => res.json())
//          .then((data) => {
 ...
 const flag = ((totalScore*50/100)<obtainedScore)
  return (
    ...
    {/* {results!==null && results.map((item)=>{
      {((results[item]?.userId===user)&&(results[item]?.week===week))?(
        <Typography> Your Result has been already record</Typography>
      ):()}
    })} */}
